...
def flip_tensor_lr(batch):
    columns = batch.size()[-1]
    device = batch.device
    index = torch.arange(columns - 1, -1, -1, device=device, dtype=torch.long)
...
def flip_tensor_ud(batch):
    rows = batch.size()[-2]
    device = batch.device
    index = torch.arange(rows - 1, -1, -1, device=device, dtype=torch.long)
    return batch.index_select(2, index)


def to_numpy(batch):
    return np.moveaxis(batch.detach().cpu().numpy(), 1, -1)
